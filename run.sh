...
if ! python -m sheetsage.infer -j --output_dir "$tmp_dir" "$audio_file"; then
    # 3. 如果失败，使用 --measures_per_chunk 4 重试
    python -m sheetsage.infer -j --output_dir "$tmp_dir" "$audio_file" --measures_per_chunk 4
fi

# 4. 创建目标目录
mkdir -p "$output_dir/leadsheet-lovelyglow_instrumental"

# 5. 复制临时文件到输出目录
cp -v "$tmp_dir"/* "$output_dir/leadsheet-lovelyglow_instrumental/"
